...
func TestFormatExtraArgsFail(t *testing.T) {
	f := cmdtesting.NewTestFactory()
	defer f.Cleanup()

	c := NewCmdFormat("kubectl", f, genericclioptions.NewTestIOStreamsDiscard())
	if validateFormatArgs(c, []string{"rc"}) == nil {
		t.Fatalf("unexpected non-error")
	}
}

func validateFormatArgs(cmd *cobra.Command, args []string) error {
	if len(args) != 0 {
...
func TestFormatFlagValidation(t *testing.T) {
	...
	for _, test := range tests {
		cmd := &cobra.Command{}
		flags := NewFormatFlags(f, genericclioptions.NewTestIOStreamsDiscard())
		flags.AddFlags(cmd)
		...
		if err != nil {
			t.Fatalf("unexpected error creating format options: %s", err)
		}
		err = o.Validate()
		if err == nil {
...
const (
	filenameCM                = "../../../testdata/format/cm.yaml"
	filenameRC                = "../../../testdata/format/rc.yaml"
	filenameRCArgs            = "../../../testdata/format/rc-args.yaml"
	filenameRCLastAppliedArgs = "../../../testdata/format/rc-lastapplied-args.yaml"
	filenameRCNoAnnotation    = "../../../testdata/format/rc-no-annotation.yaml"
	filenameRCLASTAPPLIED     = "../../../testdata/format/rc-lastapplied.yaml"
	filenameSVC               = "../../../testdata/format/service.yaml"
	filenameRCSVC             = "../../../testdata/format/rc-service.yaml"
	filenameNoExistRC         = "../../../testdata/format/rc-noexist.yaml"
	filenameRCPatchTest       = "../../../testdata/format/patch.json"
	dirName                   = "../../../testdata/format/testdir"
	filenameRCJSON            = "../../../testdata/format/rc.json"
	filenamePodGeneratedName  = "../../../testdata/format/pod-generated-name.yaml"

	filenameWidgetClientside    = "../../../testdata/format/widget-clientside.yaml"
	filenameWidgetServerside    = "../../../testdata/format/widget-serverside.yaml"
	filenameDeployObjServerside = "../../../testdata/format/deploy-serverside.yaml"
	filenameDeployObjClientside = "../../../testdata/format/deploy-clientside.yaml"
)
...
func annotateRuntimeObject(t *testing.T, originalObj, currentObj runtime.Object, kind string) (string, []byte) {
	...
	// checks that this DELETE_ME label was deleted by the format implementation in
	// kubectl.
...
func TestRunFormatPrintsValidObjectList(t *testing.T) {
	...
	cmd := NewCmdFormat("kubectl", tf, ioStreams)
...
func TestRunFormatViewLastApplied(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			/*ioStreams, _, buf, _ := genericclioptions.NewTestIOStreams()
			cmd := NewCmdFormatViewLastApplied(tf, ioStreams)
			if test.filePath != "" {
...
func TestFormatObjectWithoutAnnotation(t *testing.T) {
	cmdtesting.InitTestErrorHandler(t)
	nameRC, rcBytes := readReplicationController(t, filenameRC)
	pathRC := "/namespaces/test/replicationcontrollers/" + nameRC
...
func TestFormatObject(t *testing.T) {
	...
	for _, testingOpenAPISchema := range testingOpenAPISchemas {
		t.Run("test format when a local object is specified", func(t *testing.T) {
			tf := cmdtesting.NewTestFactory().WithNamespace("test")
			defer tf.Cleanup()
...
func TestFormatPruneObjects(t *testing.T) {
	cmdtesting.InitTestErrorHandler(t)
	nameRC, currentRC := readAndAnnotateReplicationController(t, filenameRC)
	pathRC := "/namespaces/test/replicationcontrollers/" + nameRC

	for _, testingOpenAPISchema := range testingOpenAPISchemas {
		t.Run("test format returns correct output", func(t *testing.T) {
			tf := cmdtesting.NewTestFactory().WithNamespace("test")
			defer tf.Cleanup()
...
func TestFormatObjectOutput(t *testing.T) {
	cmdtesting.InitTestErrorHandler(t)
	nameRC, currentRC := readAndAnnotateReplicationController(t, filenameRC)
	pathRC := "/namespaces/test/replicationcontrollers/" + nameRC
...
func TestFormatRetry(t *testing.T) {
	...
	for _, testingOpenAPISchema := range testingOpenAPISchemas {
		t.Run("test format retries on conflict error", func(t *testing.T) {
			firstPatch := true
			retry := false
			getCount := 0
			tf := cmdtesting.NewTestFactory().WithNamespace("test")
			...
			tf.UnstructuredClient = &fake.RESTClient{
				...
				Client: fake.CreateHTTPClient(func(req *http.Request) (*http.Response, error) {
					...
					case p == pathRC && m == "PATCH":
						if firstPatch {
							firstPatch = false
							statusErr := apierrors.NewConflict(schema.GroupResource{Group: "", Resource: "rc"}, "test-rc", fmt.Errorf("the object has been modified. Please format at first"))
			...
			if !retry || getCount != 2 {
				t.Fatalf("format didn't retry when get conflict error")
			}
...
func TestFormatNonExistObject(t *testing.T) {
	nameRC, currentRC := readAndAnnotateReplicationController(t, filenameRC)
...
func TestFormatEmptyPatch(t *testing.T) {
	...
	// 2. test format already exist object, will not send empty patch request
	ioStreams, _, buf, _ = genericclioptions.NewTestIOStreams()
	cmd = NewCmdFormat("kubectl", tf, ioStreams)
	cmd.Flags().Set("filename", filenameRC)
	cmd.Flags().Set("output", "name")
...
func TestFormatMultipleObjectsAsList(t *testing.T) {
	testFormatMultipleObjects(t, true)
}

func TestFormatMultipleObjectsAsFiles(t *testing.T) {
	testFormatMultipleObjects(t, false)
}

func testFormatMultipleObjects(t *testing.T, asList bool) {
	nameRC, currentRC := readAndAnnotateReplicationController(t, filenameRC)
	pathRC := "/namespaces/test/replicationcontrollers/" + nameRC
	...
	for _, testingOpenAPISchema := range testingOpenAPISchemas {
		t.Run("test format on multiple objects", func(t *testing.T) {
			tf := cmdtesting.NewTestFactory().WithNamespace("test")
			defer tf.Cleanup()
...
func TestFormatNULLPreservation(t *testing.T) {
	cmdtesting.InitTestErrorHandler(t)
	...
	for _, testingOpenAPISchema := range testingOpenAPISchemas {
		t.Run("test format preserves NULL fields", func(t *testing.T) {
			tf := cmdtesting.NewTestFactory().WithNamespace("test")
			defer tf.Cleanup()
...
// TestUnstructuredFormat checks format operations on an unstructured object
func TestUnstructuredFormat(t *testing.T) {
	cmdtesting.InitTestErrorHandler(t)
	...
	for _, testingOpenAPISchema := range testingOpenAPISchemas {
		t.Run("test format works correctly with unstructured objects", func(t *testing.T) {
			tf := cmdtesting.NewTestFactory().WithNamespace("test")
			defer tf.Cleanup()
...
// TestUnstructuredIdempotentFormat checks repeated format operation on an unstructured object
func TestUnstructuredIdempotentFormat(t *testing.T) {
	cmdtesting.InitTestErrorHandler(t)
	...
	for _, testingOpenAPISchema := range testingOpenAPISchemas {
		t.Run("test repeated format operations on an unstructured object", func(t *testing.T) {
			tf := cmdtesting.NewTestFactory().WithNamespace("test")
			defer tf.Cleanup()
...
func TestRunFormatSetLastApplied(t *testing.T) {
	cmdtesting.InitTestErrorHandler(t)
	nameRC, currentRC := readAndAnnotateReplicationController(t, filenameRC)
	pathRC := "/namespaces/test/replicationcontrollers/" + nameRC
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			/*ioStreams, _, buf, _ := genericclioptions.NewTestIOStreams()
			cmd := NewCmdFormatSetLastApplied(tf, ioStreams)
			cmd.Flags().Set("filename", test.filePath)
...
func TestForceFormat(t *testing.T) {
	...
	for _, testingOpenAPISchema := range testingOpenAPISchemas {
		t.Run("test format with --force", func(t *testing.T) {
			deleted := false
...
func TestDontAllowForceFormatWithServerDryRun(t *testing.T) {
...
func TestDontAllowForceFormatWithServerSide(t *testing.T) {
...
func TestDontAllowFormatWithPodGeneratedName(t *testing.T) {
	expectedError := "error: from testing-: cannot use generate name with format"
	cmdutil.BehaviorOnFatal(func(str string, code int) {
