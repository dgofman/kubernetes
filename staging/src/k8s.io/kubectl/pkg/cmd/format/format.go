...
// FormatFlags directly reflect the information that CLI is gathering via flags.  They will be converted to Options, which
...
// FormatOptions defines flags and other configuration parameters for the `format` command
type FormatOptions struct {
	...
	// format is run on these objects.
...
var (
	formatLong = templates.LongDesc(i18n.T(`
		Format a configuration to a resource by file name or stdin.
		The resource name must be specified. This resource will be created if it doesn't exist yet.
		To use 'format', always create the resource initially with either 'format' or 'create --save-config'.

		JSON and YAML formats are accepted.

		Alpha Disclaimer: the --prune functionality is not yet complete. Do not use unless you are aware of what the current state is. See https://issues.k8s.io/34274.`))

	formatExample = templates.Examples(i18n.T(`
		# Format the configuration in pod.json to a pod
		kubectl format -f ./pod.json

		# Format resources from a directory containing kustomization.yaml - e.g. dir/kustomization.yaml
		kubectl format -k dir/

		# Format the JSON passed into stdin to a pod
		cat pod.json | kubectl format -f -

		# Format the configuration from all files that end with '.json' - i.e. expand wildcard characters in file names
		kubectl format -f '*.json'

		# Note: --prune is still in Alpha
		# Format the configuration in manifest.yaml that matches label app=nginx and delete all other resources that are not in the file and match label app=nginx
		kubectl format --prune -f manifest.yaml -l app=nginx

		# Format the configuration in manifest.yaml and delete all the other config maps that are not in the file
		kubectl format --prune -f manifest.yaml --all --prune-whitelist=core/v1/ConfigMap`))

	warningNoLastAppliedConfigAnnotation = "Warning: resource %[1]s is missing the %[2]s annotation which is required by %[3]s format. %[3]s format should only be used on resources created declaratively by either %[3]s create --save-config or %[3]s format. The missing annotation will be patched automatically.\n"
	warningChangesOnDeletingResource     = "Warning: Detected changes to resource %[1]s which is currently being deleted.\n"
)

// NewFormatFlags returns a default FormatFlags
func NewFormatFlags(f cmdutil.Factory, streams genericclioptions.IOStreams) *FormatFlags {
	return &FormatFlags{
		Factory:     f,
...
func NewCmdFormat(baseName string, f cmdutil.Factory, ioStreams genericclioptions.IOStreams) *cobra.Command {

	flags := NewFormatFlags(f, ioStreams)
	...
	// format subcommands
	//cmd.AddCommand(NewCmdFormatViewLastApplied(flags.Factory, flags.IOStreams))
	//cmd.AddCommand(NewCmdFormatSetLastApplied(flags.Factory, flags.IOStreams))
	//cmd.AddCommand(NewCmdFormatEditLastApplied(flags.Factory, flags.IOStreams))

	return NewCmdFormat2(baseName, f, ioStreams)
	//return cmd
}

// AddFlags registers flags for a cli
func (flags *FormatFlags) AddFlags(cmd *cobra.Command) {
	// bind flag structs
	...
	cmdutil.AddFieldManagerFlagVar(cmd, &flags.FieldManager, FieldManagerClientSideFormat)
	...
	cmd.Flags().BoolVar(&flags.Prune, "prune", flags.Prune, "Automatically delete resource objects, that do not appear in the configs and are created by either format or create --save-config. Should be used with either -l or --all.")
...
func (flags *FormatFlags) ToOptions(cmd *cobra.Command, baseName string, args []string) (*FormatOptions, error) {
	if len(args) != 0 {
		return nil, cmdutil.UsageErrorf(cmd, "Unexpected args: %v", args)
	}

	serverSideFormat := cmdutil.GetServerSideApplyFlag(cmd)
	...
	fieldManager := GetFormatFieldManagerFlag(cmd, serverSideFormat)
	...
	o := &FormatOptions{
		...
		ToPrinter:        toPrinter,
		ServerSideFormat: serverSideFormat,
...
// Validate verifies if FormatOptions are valid and without conflicts.
func (o *FormatOptions) Validate() error {
	if o.ForceConflicts && !o.ServerSideFormat {
		return fmt.Errorf("--force-conflicts only works with --server-side")
	}

	if o.DryRunStrategy == cmdutil.DryRunClient && o.ServerSideFormat {
		return fmt.Errorf("--dry-run=client doesn't work with --server-side (did you mean --dry-run=server instead?)")
	}

	if o.ServerSideFormat && o.DeleteOptions.ForceDeletion {
...
// GetObjects returns a (possibly cached) version of all the valid objects to format
...
// the FormatOptions is filled in and valid.
func (o *FormatOptions) GetObjects() ([]*resource.Info, error) {
	var err error = nil
	if !o.objectsCached {
		r := o.Builder.
			Unstructured().
...
func (o *FormatOptions) SetObjects(infos []*resource.Info) {
	o.objects = infos
	o.objectsCached = true
}

// Run executes the `format` command.
func (o *FormatOptions) Run() error {
	if o.PreProcessorFn != nil {
		klog.V(4).Infof("Running format pre-processor function")
	...
	if len(infos) == 0 && len(errs) == 0 {
		return fmt.Errorf("no objects passed to format")
	}
	// Iterate through all objects, formating each one.
	for _, info := range infos {
		if err := o.formatOneObject(info); err != nil {
			errs = append(errs, err)
		}
	}
	// If any errors occurred during format, then return error (or
	// aggregate of errors).
	if len(errs) == 1 {
		return errs[0]
	}
	...
	if o.PostProcessorFn != nil {
		klog.V(4).Infof("Running format post-processor function")
...
func (o *FormatOptions) formatOneObject(info *resource.Info) error {
	...
	if len(info.Name) == 0 {
		...
		if len(generatedName) > 0 {
			return fmt.Errorf("from %s: cannot use generate name with format", generatedName)
		}
	}
	...
	if o.ServerSideFormat {
		// Send the full object to be applied on the server side.
		data, err := runtime.Encode(unstructured.UnstructuredJSONScheme, info.Object)
		if err != nil {
			return cmdutil.AddSourceToErr("serverside-format", info.Source, err)
		}
		...
		if err != nil {
			if isIncompatibleServerError(err) {
				err = fmt.Errorf("Server-side format not available on the server: (%v)", err)
			}
			if errors.IsConflict(err) {
				err = fmt.Errorf(`%v
Please review the fields above--they currently have other managers. Here
are the ways you can resolve this warning:
* If you intend to manage all of these fields, please re-run the format
...
See https://kubernetes.io/docs/reference/using-api/server-side-format/#conflicts`, err)
			}
			return err
		}
	...
	if err := info.Get(); err != nil {
		...
		// First, update the annotation used by kubectl format
	...
	if o.DryRunStrategy != cmdutil.DryRunClient {
		...
		if err != nil {
			return cmdutil.AddSourceToErr(fmt.Sprintf("formating patch:\n%s\nto:\n%v\nfor:", patchBytes, info), info.Source, err)
		}
...
func (o *FormatOptions) shouldPrintObject() bool {
...
func (o *FormatOptions) printObjects() error {
...
func (o *FormatOptions) MarkNamespaceVisited(info *resource.Info) {
	if info.Namespaced() {
...
func (o *FormatOptions) MarkObjectVisited(info *resource.Info) error {
...
func (o *FormatOptions) PrintAndPrunePostProcessor() func() error {
...
const (
	// FieldManagerClientSideFormat is the default client-side format field manager.
	//
	// The default field manager is not `kubectl-format` to distinguish from
	// server-side format.
	FieldManagerClientSideFormat = "kubectl-client-side-format"
	// The default server-side format field manager is `kubectl`
	// instead of a field manager like `kubectl-server-side-format`
	// for backward compatibility to not conflict with old versions
	// of kubectl server-side format where `kubectl` has already been the field manager.
	fieldManagerServerSideFormat = "kubectl"
)

// GetFormatFieldManagerFlag gets the field manager for kubectl format
// if it is not set.
//
// The default field manager is not `kubectl-format` to distinguish between
// client-side and server-side format.
func GetFormatFieldManagerFlag(cmd *cobra.Command, serverSide bool) string {
	...
	if serverSide {
		return fieldManagerServerSideFormat
	}

	return FieldManagerClientSideFormat
}
